<script lang="ts">
    ...
    import {t} from '@/motis-helpers/i18n/translation';
    ...
    const {
        ...
        itinerary: Itinerary;
        // In future implementations of the ConnectionDetail component, the following props can be used for triggering the route visualization on the map
        // onClickStop can be used to trigger the route visualization on the map for the stretch goal
        //onClickStop: (name: string, stopId: string, time: Date) => void;
        //onClickTrip: (tripId: string) => void;
        shadowItinerary: ItineraryShadow;
    } = $props();
    const onClickStop = (name: string, stopId: string, time: Date) => {
        console.log(name, stopId, time);
    };
...
<div class="text-lg">
    {#each itinerary.legs as l, i}
        {@const shadowLeg = shadowItinerary.legs[i]}
        ...
        {#if l.routeShortName}
            <div class="w-full flex justify-between items-center space-x-1">
                <Route {onClickTrip} {l} shadowLeg={shadowItinerary.legs[i]}/>
